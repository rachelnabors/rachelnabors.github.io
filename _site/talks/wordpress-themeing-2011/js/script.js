...
$(document).ready(function() {
 ...
 //make each section the height of the window if it is horter than it
 ...
 //menu
	...
	//give each section a sequential id
	$("section").each(function(index) {
	  $(this).attr('id', ('part' + index));
	});
	
	//make nav lists
	$('.wrapper').append('<nav role="navigation" id="toc"><h4><abbr title="Table of Contents">TOC</abbr></h4><ol></ol></nav><nav id="next-prev"><ul class="group"><li id="previous"></li><li id="next"></li><li class="follow"><a href="http://RachelNabors.com/wordpress-themeing-2011">RachelNabors.com/wordpress-themeing-2011</a></li></ul></nav>');
	
	//get the h2 from each section
	$("h2").each(function(){
	//add them to a list
		$(this).clone()
		.replaceWith("<li><a href=''>" + $(this).text() + "</a></li>").appendTo("#toc ol");
	});
	//link each h2 to its section
	$("#toc a").each(function(index) {
	  $(this).attr('href', ('#part' + index));
	});
	//rename first link
	$("#toc li:first a").text('Modern WordPress Themeing');
